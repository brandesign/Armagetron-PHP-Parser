...
class StyCt extends Parser implements ParserInterface
{
    ...
    public function setEventDefinitions()
    {
        ...
        LadderLog::getInstance()->getEventCollection()
            ...
            ->add('BASEZONE_CONQUERER',       array('player:player', 'percent_won:float'))
            ->add('BASEZONE_CONQUERER_TEAM',  array('team:team', 'score:int'))
            ->add('BASE_ENEMY_RESPAWN',       array('respawner:player', 'player_respawned:player'))
            ->add('BASE_RESPAWN',             array('respawner:player', 'player_respawned:player'))
